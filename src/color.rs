...
mod tests {
    ...
    #[test]
    fn black_converts_to_lab_zero() {
        let lab = rgb_to_lab(RgbColor { r: 0, g: 0, b: 0 });
        assert!(
            lab.l.abs() < 0.01,
    ...
    #[test]
    fn white_converts_to_lab_100() {
        let lab = rgb_to_lab(RgbColor {
            r: 255,
            g: 255,
            b: 255,
        });
    ...
    #[test]
    fn is_dark_corrected() {
        let black = rgb_to_lab(RgbColor { r: 0, g: 0, b: 0 });
        assert!(black.is_dark(), "Black should be dark (L* = {})", black.l);

        let white = rgb_to_lab(RgbColor {
            r: 255,
            g: 255,
            b: 255,
        });
    ...
    fn round_trip_rgb_lab_rgb() {
        ...
        let recovered = lab_to_rgb(lab);
        let diff = |a: u8, b: u8| (a as i16 - b as i16).unsigned_abs();
        assert!(diff(original.r, recovered.r) <= 2);
        assert!(diff(original.g, recovered.g) <= 2);
        assert!(diff(original.b, recovered.b) <= 2);
    }
}
