...
mod tests {
    ...
    #[test]
    fn uniform_image_has_low_entropy() {
        let lab: Vec<LabColor> = (0..100)
            .map(|_| LabColor {
                l: 50.0,
                a: 0.0,
                b: 0.0,
            ...
            .collect();
        let h = shannon_entropy(&lab);
        assert!(
            h < 0.01,
            "Uniform image should have near-zero entropy, got {}",
            h
        );
    }

    #[test]
    fn grey_image_has_low_colorfulness() {
        let pixels: Vec<RgbColor> = (0..100)
            .map(|_| RgbColor {
                r: 128,
                g: 128,
                b: 128,
            ...
            .collect();
        let m = hasler_suesstrunk(&pixels);
        assert!(
            m < 1.0,
            "Grey image should have near-zero colorfulness, got {}",
            m
        );
    }
    ...
    fn orientation_classification() {
        ...
        assert!(matches!(
            classify_orientation(1080, 1920),
            Orientation::Portrait
        ));
        assert!(matches!(
            classify_orientation(512, 512),
            Orientation::Square
        ));
    }
}
